...
public class CcsidUtils {
    /**
     * Contains a copy of the toolbox's encoding-to-CCSID map, except with
     * typesafety built in for accessors of
     * this map. This map has the following advantages over the toolbox map:
     * <ul>
     * <li>All keys are lowercased, so lookups are expected to be done with
     * lowercase strings. This is because the JDK names don't always case-match
     * Toolbox names.
     * <li>The values are integers. The toolbox map uses string representations of
     * integers.
     * <li>We can easily add more as we discover them. The toolbox, unsurprisingly,
     * misses some (see comment in code in static initializer for that list)
     * </ul>
     */
    private static final Map<String, Integer> g_toolboxEncodingMap;
    static {
        g_toolboxEncodingMap = new Hashtable<>();
        // load the toolbox's lookup table into our hashmap with type-safety (and do it
        // on static init so the reflection is only needed once)
        try {
    ...
    public static Charset ccsidToCharset(final int _ccsid) throws UnsupportedCharsetException {
        ...
        // The above technique misses a few that can be picked up by simply looking up
        // the CCSID#
        // These include:
        // 5601=EUC-KR
        // 646=US-ASCII
    ...
    public static int encodingToCCSID(final String _encoding) {
        ...
        ccsid = NLS.encodingToCCSID(_encoding.replace("ISO-", "ISO").replace("-", "_")); // this is a case-sensitive
                                                                                         // lookup
        if (0 < ccsid) {
            return ccsid;
        }
        try {
            // Some names already contain the exact mapping to codepage. "Cp____" and
            // "IBM____" always seem to have a ccsid of ____
        ...
        final Integer lookedUp = g_toolboxEncodingMap.get(_encoding.toLowerCase()); // case-insensitive. All values of
                                                                                    // the map are lowercased
    ...
    public static String unknownStringToEncoding(final String _enc, final String _fallback)
        ...
        try {
            ...
            // TODO: it's not a charset name, not an encoding name, not a CCSID #, what is
            // it?
            return _fallback;
        }
    }
